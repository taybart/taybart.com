...
func (c StorageClient) List() ([]string, error) {
	...
	for object := range ch {
		if object.Err != nil {
			err := fmt.Errorf("could not get note %s: %+v", object.Key, object.Err)
			return res, err
		}
...
func (c StorageClient) Get(id string) (string, error) {
	...
	if err != nil {
		err = fmt.Errorf("could not get note %s: %+v", id, err)
		return "", err
	}
	b, err := ioutil.ReadAll(obj)
	if err != nil {
		err = fmt.Errorf("could not read note %s: %+v", id, err)
		return "", err
	}
	return string(b), nil
}
func (c StorageClient) Set(id string, note string) error {
	...
	if err != nil {
		return fmt.Errorf("could not get note %s: %+v", id, err)
	}
	fmt.Printf("%+v\n", info)
	return nil
}
